...
/*   Updated: 2025/12/10 12:06:33 by maalwis          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	std::cout << PURPLE "ClapTrap " << _name << " default constructor called" RESET << std::endl;
}
...
{
	std::cout << ROSE "ClapTrap copy constructor called\n" RESET;
...
{
	std::cout << ROSE "ClapTrap copy assignement operator called\n" RESET;
	if (this != &other)
	{
...
std::string ClapTrap::colorName(const std::string &name) const
{
	...
	if (name == "Baby_clap_name")
		return (ORANGE + _name + RESET);
	return (name);
}
